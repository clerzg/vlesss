...
cat <<EOF > ${CONFIG_FILE}
{"log":{"disabled":true},"inbounds":[{"type":"vmess","listen":"::","listen_port":${PORT},"users":[{"uuid":"${UUID}"}],"transport":{"type":"ws"}}],"outbounds":[{"type":"direct"}]}
EOF
...
echo ""
echo "🚀节点链接："
echo "vmess://${UUID}@${IP}:${PORT}?type=ws&host=gw.alicdn.com#${LOC}"
